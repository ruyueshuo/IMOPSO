function  [best_fuzzy , best_topsis] = imopsofunc 
%{
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%  MATLAB Code for                                                  %
%                                                                   %
...
%         Vg   = mpc_data.gen(:, VG);
%         for i = 1:length(gbus)
%             Vmin(i) = mpc_data.bus(gbus(i), VMIN);
%             Vmax(i) = mpc_data.bus(gbus(i), VMAX);
%         end
...
idx_fuzzy = find(w==(max(w)));
best_fuzzy = rep_costs(:,idx_fuzzy);
...
%% TOPSIS
 ...
 d = dmax./(dmax+dmin);
 idx_topsis = find(d==(max(d)));
 best_topsis = rep_costs(:,idx_topsis);
...
%     mpc.gen(i,QG) = cost_best_position(nVar/2+i);
% end 
